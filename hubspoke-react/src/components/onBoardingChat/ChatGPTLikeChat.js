...
export default function ChatGPTLikeChat() {
  ...
  const postAnswer = (payload) => {
    ...
    const pendingTimer = setTimeout(() => {
      setIsPending(false);
      appendMessages([{ role: "assistant", content: "Network seems slow—please, be with us." }]);
    }, 12000);
    ...
    <div className="chat-wrap">
      <div className="chat" role="application" aria-label="Onboarding chat">
        ...
        <div className={`composer-wrap ${showNudge ? "nudge" : ""}`}>
          {currentHint && !isPending && !completed && (
            <div className="hint">
              {currentHint}
        ...
        {currentQ?.mode === "chips+input" && !isPending && !completed && (
          ...
          {currentQ?.multi && !isPending && !completed && (selChips.length > 0 || customChips.length > 0) && (
